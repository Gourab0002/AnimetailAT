...
class PagerPageHolder(
    ...
    private val translationService: PageTranslationService by injectLazy()
            ...
            val (source, isAnimated, background) = withIOContext {
                streamFn().buffered(16).use { source ->
                    ...
                    if (extraPage != null) {
                        ...
                        val readyForTranslation = extraPage == null || source2 != null
                        val displaySource = if (!isAnimated && readyForTranslation) {
                            translationService.translatePage(
                                itemSource,
                                viewer.activity.viewModel.currentSource?.lang,
                            )
                        } else {
                            itemSource
                        }
                        Triple(displaySource, isAnimated, background)
                    }
                }
            }
            withUIContext {

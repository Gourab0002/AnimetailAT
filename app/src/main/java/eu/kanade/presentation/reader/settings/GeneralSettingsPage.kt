...
internal fun ColumnScope.GeneralPage(viewModel: ReaderSettingsViewModel) {
    ...
    CheckboxItem(
        label = stringResource(TLMR.strings.pref_reader_translate),
        checked = translationEnabled,
        onClick = {
            if (!translationEnabled && !translationPreferences.hasCredentials()) {
                context.toast(TLMR.strings.translation_missing_api_key)
            } else {
                translationPreferences.enabled.toggle()
            }
        },
    )
